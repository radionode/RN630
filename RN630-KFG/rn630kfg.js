...
function Decoder(bytes, port) {
    ...
    if (head === 11) {
      ...
      o2 = parseInt(readUInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (o2 >= 65535) o2 = null;
      offset += raw_size;

      h2s = parseInt(readUInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (h2s >= 65535) h2s = null;
      offset += raw_size;
      ...
      sos_mandown = parseInt(readInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (sos_mandown >= 65535) sos_mandown = null;
      offset += raw_size;

      feelliketemp = parseInt(readInt16LE(data.slice(offset, offset + raw_size)), 10) / 100;
      if (feelliketemp <= -9999.0) feelliketemp = null;
